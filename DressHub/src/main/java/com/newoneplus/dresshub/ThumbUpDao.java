...
public class ThumbUpDao {
    ...
    public ArrayList<Integer> getLikeProductIdList(int userId){
            ...
            productIds = (ArrayList<Integer>) jdbcTemplate.queryForObject(
                    "SELECT * FROM THUMB_UP WHERE PRODUCT = ?", params, (RowMapper) (rs, rowNum) -> {
                        ...
                        while(rs.next()){
                            ...
                            arrayList.add(rs.getInt("PRODUCT_ID"));
                        }
                        return arrayList;
                    });
        }catch (EmptyResultDataAccessException e){
    ...
    public Integer insert(int userId, int productId) {
        ...
        jdbcTemplate.update(con -> {
            ...
            PreparedStatement preparedStatement = con.prepareStatement(
                    //TODO 아직 PRODUCT ID가 이름이 없음  , 왜래키 그냥 이대로 가져올 수 있는지도 모름
                    "INSERT THUMB_UP(USER_ID, PRODUCT_ID) VALUES (?, ?)",
    ...
    public void delete(int userId, int productId){
        Object[] params = {userId, productId};
        jdbcTemplate.update("DELETE FROM THUMB_UP WHERE USER_ID = ? AND PRODUCT_ID = ?", params);
    }
