...
public class ProductController {
    ...
    public String insert(Product product){
        ...
        if(!product.getThumbnailImage().isEmpty()){
            filename = product.getThumbnailImage().getOriginalFilename();
            // 개발디렉토리 - 파일 업로드 경로
            //String path = "C:\\Users\\doubles\\Desktop\\workspace\\gitSpring\\"
            //                + "spring02\\src\\main\\webapp\\WEB-INF\\views\\images";
            // 배포디렉토리 - 파일 업로드 경로
            String path = "C:\\workspace\\project\\DressHub\\DressHub\\src\\main\\resources\\static\\thumbnailImage\\";
            try {
                new File(path).mkdirs(); // 디렉토리 생성
                // 임시디렉토리(서버)에 저장된 파일을 지정된 디렉토리로 전송
                product.getThumbnailImage().transferTo(new File(path+filename));
            } catch (Exception e) {
                e.printStackTrace();
            }
