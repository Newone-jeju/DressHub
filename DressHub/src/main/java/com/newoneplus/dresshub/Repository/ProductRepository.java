...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    @Query(value = "select p from Product p inner join ThumbUp t on (p.id = t.productId)and (t.userId = :user) ")
    Page<Product> findAllByUser(@Param("user")String user, Pageable pageable);
    @Query(value="select p from Product p left outer join ThumbUp t on (p.id=t.productId) and t.userId =:user where category like :category%")
