...
//        String sql = "update user set password = ?, name = ?, email = ?, address = ?, phone_number = ? ," +
//                "user_type = ?, nickname = ?, introduce = ?, open_private_info = ? , certification = ? , resister_date = ?" +
//                "where id = ?";
//        Object[] params = new Object[]{user.getPassword(), user.getName(), user.getEmail(), user.getAddress(),
//                user.getPhoneNumber(), user.getUserType(), user.getNickname(), user.getIntroduce(), user.isOpenPrivateInfo(),
//                user.isCertification(), user.getResisterDate(), user.getId()};
//        jdbcTemplate.update(sql, params);
//
//    }
//}
